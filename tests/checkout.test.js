...
test('checkout accepts non-finite numeric price', async () => {
  const response = await handler({
    httpMethod: 'POST',
...
test('checkout rejects malformed NaN price literal payload', async () => {
  const response = await handler({
    httpMethod: 'POST',
    body: '{"cart":{"items":[{"type":"pizza","id":"margherita","price":NaN}]}}'
  });
